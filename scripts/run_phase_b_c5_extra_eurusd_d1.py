"""Pivot v4 Phase B C5 Extra — EURUSD D1 walk-forward simple (sans méta-labeling).

Flow simplifié :
1. Charge EURUSD D1, cutoff train ≤ 2022-12-31, test ≥ 2024-01-01.
2. Construit le superset de features, sélectionne le top 15 C5 pour EURUSD D1.
3. Génère la target Donchian (N=20, M=20) sur train → backtest → trades binaires
   (winner = pips_net > 0).
4. Entraîne stacking (RF+HGBM+LogReg, defaults sklearn) sur les features aux barres
   d'entrée des trades train.
...
# Hyperparams par défaut pour stacking (pas de tuning C4)
...
def _build_features_for_split(df_split: pd.DataFrame) -> pd.DataFrame:
    """Construit le superset et sélectionne le top 15 C5 pour EURUSD D1."""
...
def _train_stacking_model(
    ...
    """Entraîne le modèle stacking (RF+HGBM+LogReg) avec les defaults sklearn."""
...
def _generate_model_signals(
    ...
    """Génère les signaux directionnels (1=LONG, -1=SHORT) depuis le modèle stacking.
...
def main() -> int:
    ...
    # ── 5. Entraînement stacking ─────────────────────────────────────────
    print(f"\nEntraînement stacking sur {len(X_train)} trades train...")
    ...
    print(f"\n{'='*60}")
    print(f"Phase B C5 Extra — EURUSD D1 terminé.")
