"""Pivot v4 Phase B C5 Extra â€” EURUSD D1 walk-forward simple (sans mÃ©ta-labeling).

Flow simplifiÃ© :
1. Charge EURUSD D1, cutoff train â‰¤ 2022-12-31, test â‰¥ 2024-01-01.
2. Construit le superset de features, sÃ©lectionne le top 15 C5 pour EURUSD D1.
3. GÃ©nÃ¨re la target Donchian (N=20, M=20) sur train â†’ backtest â†’ trades binaires
   (winner = pips_net > 0).
4. EntraÃ®ne stacking (RF+HGBM+LogReg, defaults sklearn) sur les features aux barres
   d'entrÃ©e des trades train.
...
8. Sauvegarde dans predictions/phase_b_c5_extra_eurusd_d1.json.
...
import numpy as np
import pandas as pd
from app.models.candidates import TimeSeriesHoldoutStacking  # noqa: E402  # type-hint
...
# Hyperparams par dÃ©faut pour stacking (pas de tuning C4)
...
def _build_features_for_split(df_split: pd.DataFrame) -> pd.DataFrame:
    """Construit le superset et sÃ©lectionne le top 15 C5 pour EURUSD D1."""
...
def _train_stacking_model(
    ...
    y_train: pd.Series,
) -> TimeSeriesHoldoutStacking:
    """EntraÃ®ne le modÃ¨le stacking (RF+HGBM+LogReg) avec les defaults sklearn."""
...
def _generate_model_signals(
    df: pd.DataFrame,
    model: TimeSeriesHoldoutStacking,
...
def main() -> int:
    ...
    # â”€â”€ 1. Chargement EURUSD D1 â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ 5. EntraÃ®nement stacking â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    print(f"\nEntraÃ®nement stacking sur {len(X_train)} trades train...")
    model = _train_stacking_model(X_train, y_train)
    ...
    print(f"\n{'='*60}")
    print(f"Phase B C5 Extra â€” EURUSD D1 terminÃ©.")
    ...
    out: dict[str, Any] = {
        "hypothesis": "C5_EXTRA_EURUSD_D1",
        "phase": "Phase B C5 Extra",
        "asset": ASSET,
        "tf": TF,
        "model": "stacking",
        "hyperparams": {"rf": "default", "hgbm": "default", "meta": "LogisticRegression", "cv": 5},
        "threshold": STACKING_THRESHOLD,
    ...
    out_path = Path("predictions/phase_b_c5_extra_eurusd_d1.json")
