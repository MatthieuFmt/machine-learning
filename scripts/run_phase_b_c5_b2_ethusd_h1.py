"""Pivot v4 Phase B C5 — B2 : ETHUSD H1 walk-forward simple (sans méta-labeling).

Flow simplifié :
1. Charge ETHUSD H1, cutoff train ≤ 2022-12-31, test ≥ 2024-01-01.
2. Construit le superset de features, sélectionne le top 15 C5 pour ETHUSD H1.
3. Génère la target Donchian (N=50, M=20) sur train → backtest → trades binaires
   (winner = pips_net > 0).
4. Entraîne hgbm (hyperparams C5) sur les features aux barres d'entrée des trades train.
...
8. Sauvegarde dans predictions/phase_b_c5_b2_ethusd_h1.json.
...
def _build_features_for_split(df_split: pd.DataFrame) -> pd.DataFrame:
    """Construit le superset et sélectionne le top 15 C5 pour ETHUSD H1."""
...
def _generate_donchian_signals(df: pd.DataFrame) -> pd.Series:
    """Génère les signaux Donchian (N=50, M=20) pour H1."""
...
def _train_hgbm_model(
    ...
    """Entraîne le modèle hgbm avec les hyperparams C5 pour ETHUSD H1."""
...
def main() -> int:
    ...
    # ── 5. Entraînement hgbm ─────────────────────────────────────────────
    print(f"\nEntraînement hgbm sur {len(X_train)} trades train...")
    ...
    print(f"\n{'='*60}")
    print(f"Phase B C5 — B2 ETHUSD H1 terminé.")
    ...
    out_path = Path("predictions/phase_b_c5_b2_ethusd_h1.json")
