"""Inspecte les CSV US30 D1/H4 — validation préalable au pipeline v2.

Usage : python scripts/inspect_us30_csv.py
"""
...
OPTIONAL_COLS = {"Volume", "TickVol", "Spread"}
CSV_FILES = ["data/USA30IDXUSD_D1.csv", "data/USA30IDXUSD_H4.csv"]
...
def inspect_csv(path: str) -> pd.DataFrame | None:
    ...
    try:
        df = pd.read_csv(
            file_path,
            sep="\t",
            names=["Time", "Open", "High", "Low", "Close", "TickVol", "Volume"],
            skiprows=1,
        )
    ...
    if missing:
        logger.error(
            "%s : colonnes obligatoires manquantes : %s", file_path.name, sorted(missing)
        )
        return None

    has_volume = bool(OPTIONAL_COLS & cols)
    if not has_volume:
        logger.warning("%s : pas de colonne Volume/TickVol — Volume_Ratio sera indisponible", file_path.name)
    ...
    if not df.index.is_monotonic_increasing:
        logger.error("%s : index temporel NON monotone — doublons détectés", file_path.name)
    ...
    volum_col = next((c for c in OPTIONAL_COLS if c in cols), None)
    vol_zero_rate = (df[volum_col] == 0).mean() * 100 if volum_col else None
...
def main() -> int:
    setup_logging()
    logger.info("=== Inspection CSV US30 D1 / H4 ===")
    ...
    for csv_path in CSV_FILES:
        ...
        if df is not None:
            ...
            cleaned_name = f"USA30IDXUSD_{tf}_cleaned.csv"
            save_cleaned(df, cleaned_name)

    if not any_valid:
        logger.error("=" * 60)
        logger.error("DONNÉES US30 INDISPONIBLES")
        logger.error("Aucun CSV US30 trouvé dans data/")
        logger.error("Fallback suggéré : XAUUSD H4 ou GER30 D1")
