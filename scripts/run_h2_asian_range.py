"""Phase H2 — Backtest Asian Range Breakout sur EURUSD/GBPUSD/USDJPY H1.

Stratégie déterministe (Étape 1 cascade) : long/short symétriques selon
breakout du range Tokyo [00:00-06:00 UTC] (7 barres), évalué à la close
de la barre 07:00 UTC. TP = 1.5×range, SL = 0.5×range, time-stop à 22:00 UTC.

Train ≤ 2022-12-31, OOS ≥ 2024-01-01.

Critères GO par actif (Étape 1) :
    - Sharpe OOS ≥ 0.7
    - Mean PnL OOS > 0
    - p-value bootstrap < 0.10
    - n_trades OOS ≥ 30 (minimum statistique sur 2.5 ans)

Si ≥ 1 actif GO, on bascule sur Étape 2 cascade (ML méta-labeling) —
volume de trades estimé (~50-150/an × 13 ans train) compatible avec ML
robuste, contrairement à Pre-FOMC.
"""
...
# Hyperparams fixés (pas de tuning OOS — 0 n_trial sur la grille)
TP_MULT = 1.5
SL_MULT = 0.5
...
def _run_one_asset(asset: str) -> dict[str, Any]:
    ...
    # ── Simulation Train ─────────────────────────────────────────────
    print("\n  ── TRAIN ──")
    trades_train = simulate_asian_range_trades(
        df_train, cfg,
    ...
    # ── Simulation OOS ───────────────────────────────────────────────
    print("\n  ── TEST (OOS) ──")
    trades_test = simulate_asian_range_trades(
        df_test, cfg,
        tp_mult=TP_MULT, sl_mult=SL_MULT,
        time_stop_hour_utc=TIME_STOP_HOUR_UTC,
    )
    ...
    go = go_sharpe and go_mean and go_pvalue and go_ntrades
    ...
    return {
        ...
        "sl_mult": SL_MULT,
        "time_stop_hour_utc": TIME_STOP_HOUR_UTC,
        "tokyo_window_utc": "[00:00, 06:00] (7 barres, hors signal_hour 07:00)",
...
def main() -> int:
    set_global_seeds()
    print("=" * 70)
    print("PHASE H2 — Asian Range Breakout (Étape 1 cascade — déterministe)")
    print(f"Actifs : {ASSETS} sur {TF}")
    print(f"Range Tokyo = [00:00, 06:00] UTC (7 barres) | signal close 07:00")
    print(f"TP = 1.5×range, SL = 0.5×range | Time-stop Close 22:00 UTC")
    ...
    for asset in ASSETS:
        try:
            ...
            logger.exception("asian_range_run_failed", extra={"context": {"asset": asset}})
            print(f"\n  ⚠️ {asset} ÉCHEC : {exc}")
            all_results[asset] = {"asset": asset, "error": str(exc), "go": False}

    # ── Sauvegarde 1 JSON par actif ─────────────────────────────────────
    out_dir = Path("predictions")
    out_dir.mkdir(parents=True, exist_ok=True)
    for asset, res in all_results.items():
        out_path = out_dir / f"h2_asian_range_{asset.lower()}.json"
        out_path.write_text(
    ...
    # ── Synthèse multi-actif ────────────────────────────────────────────
    print("\n" + "=" * 70)
    print("SYNTHÈSE PHASE H2")
    print("=" * 70)
    print(f"{'Actif':<8} {'Sharpe OOS':>10} {'Mean OOS':>10} {'n_OOS':>6} {'p-val':>6} {'GO?':>4}")
    print("─" * 50)
    for asset, res in all_results.items():
        if "error" in res:
        ...
        print(f"{asset:<8} {m['sharpe']:>+10.2f} {m['mean_pnl']:>+10.1f} "
              f"{m['n_trades']:>6d} {m['p_value_bootstrap']:>6.3f} {go_mark:>4}")
    ...
    if n_go > 0:
        ...
        print(f"  ==> 0 actif GO — bascule H3 (NR4/NR7) ou re-spec H2 nécessaire")
