"""Pivot v4 Phase B C5 Extra â€” BTCUSD D1 walk-forward simple (sans mÃ©ta-labeling).

Flow simplifiÃ© :
1. Charge BTCUSD D1, cutoff train â‰¤ 2022-12-31, test â‰¥ 2024-01-01.
2. Construit le superset de features, sÃ©lectionne le top 15 C5 pour BTCUSD D1.
3. GÃ©nÃ¨re la target Donchian (N=20, M=20) sur train â†’ backtest â†’ trades binaires
   (winner = pips_net > 0).
4. EntraÃ®ne hgbm (defaults sklearn, exclu C4) sur les features aux barres d'entrÃ©e
   des trades train.
...
8. Sauvegarde dans predictions/phase_b_c5_extra_btcusd_d1.json.

ðŸš« Pas de mÃ©ta-labeling. Pas de walk-forward complexe. Train une fois, test une fois.
Note : BTCUSD D1 exclu de C4 (Sharpe 0.28) â†’ defaults sklearn hgbm.
"""
...
# BTCUSD D1 exclu de C4 â†’ defaults sklearn hgbm
HGBM_DEFAULTS = {
    "learning_rate": 0.1,
    "max_depth": None,
    "max_leaf_nodes": 31,
...
def _build_features_for_split(df_split: pd.DataFrame) -> pd.DataFrame:
    """Construit le superset et sÃ©lectionne le top 15 C5 pour BTCUSD D1."""
...
def _generate_donchian_signals(df: pd.DataFrame) -> pd.Series:
    """GÃ©nÃ¨re les signaux Donchian (N=20, M=20) pour D1."""
...
def _train_hgbm_model(
    X_train: pd.DataFrame,
    y_train: pd.Series,
) -> HistGradientBoostingClassifier:
    """EntraÃ®ne le modÃ¨le hgbm avec les defaults sklearn (BTCUSD exclu C4)."""
    model = HistGradientBoostingClassifier(
        learning_rate=HGBM_DEFAULTS["learning_rate"],
        max_depth=HGBM_DEFAULTS["max_depth"],
        max_leaf_nodes=HGBM_DEFAULTS["max_leaf_nodes"],
        min_samples_leaf=HGBM_DEFAULTS["min_samples_leaf"],
...
def main() -> int:
    ...
    # â”€â”€ 1. Chargement BTCUSD D1 â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ 5. EntraÃ®nement hgbm â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    print(f"\nEntraÃ®nement hgbm (defaults sklearn) sur {len(X_train)} trades train...")
    model = _train_hgbm_model(X_train, y_train)
    ...
    print(f"\n{'='*60}")
    print(f"Phase B C5 Extra â€” BTCUSD D1 terminÃ©.")
    ...
    out: dict[str, Any] = {
        "hypothesis": "C5_EXTRA_BTCUSD_D1",
        "phase": "Phase B C5 Extra",
        "asset": ASSET,
        "tf": TF,
        "model": "hgbm",
        "note": "exclu C4, Sharpe 0.28, defaults sklearn",
        "hyperparams": HGBM_DEFAULTS,
        "threshold": THRESHOLD,
    ...
    out_path = Path("predictions/phase_b_c5_extra_btcusd_d1.json")
