"""Pivot v4 Phase B C5 Extra — BTCUSD D1 walk-forward simple (sans méta-labeling).

Flow simplifié :
1. Charge BTCUSD D1, cutoff train ≤ 2022-12-31, test ≥ 2024-01-01.
2. Construit le superset de features, sélectionne le top 15 C5 pour BTCUSD D1.
3. Génère la target Donchian (N=20, M=20) sur train → backtest → trades binaires
   (winner = pips_net > 0).
4. Entraîne hgbm (defaults sklearn, exclu C4) sur les features aux barres d'entrée
   des trades train.
...
Note : BTCUSD D1 exclu de C4 (Sharpe 0.28) → defaults sklearn hgbm.
"""
...
# BTCUSD D1 exclu de C4 → defaults sklearn hgbm
HGBM_DEFAULTS = {
    "learning_rate": 0.1,
    "max_depth": None,
    "max_leaf_nodes": 31,
...
def _build_features_for_split(df_split: pd.DataFrame) -> pd.DataFrame:
    """Construit le superset et sélectionne le top 15 C5 pour BTCUSD D1."""
...
def _generate_donchian_signals(df: pd.DataFrame) -> pd.Series:
    """Génère les signaux Donchian (N=20, M=20) pour D1."""
...
def _train_hgbm_model(
    ...
    """Entraîne le modèle hgbm avec les defaults sklearn (BTCUSD exclu C4)."""
...
def _generate_model_signals(
    ...
    """Génère les signaux directionnels (1=LONG, -1=SHORT) depuis le modèle hgbm.

    Le modèle prédit P(win) à chaque barre. Les signaux sont générés quand
    prob > threshold. La direction est déduite des features de tendance
...
def main() -> int:
    ...
    # ── 5. Entraînement hgbm ─────────────────────────────────────────────
    print(f"\nEntraînement hgbm (defaults sklearn) sur {len(X_train)} trades train...")
    ...
    print(f"\n{'='*60}")
    print(f"Phase B C5 Extra — BTCUSD D1 terminé.")
