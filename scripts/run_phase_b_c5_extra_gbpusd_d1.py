"""Pivot v4 Phase B C5 Extra — GBPUSD D1 walk-forward simple (sans méta-labeling).

Flow simplifié :
1. Charge GBPUSD D1, cutoff train ≤ 2022-12-31, test ≥ 2024-01-01.
2. Construit le superset de features, sélectionne le top 15 C5 pour GBPUSD D1.
...
def _build_features_for_split(df_split: pd.DataFrame) -> pd.DataFrame:
    """Construit le superset et sélectionne le top 15 C5 pour GBPUSD D1."""
...
def _train_rf_model(
    ...
    """Entraîne le modèle RandomForest avec les hyperparams C5 pour GBPUSD D1."""
...
def _generate_model_signals(
    ...
    Le modèle ne fait que rejeter ou conserver, jamais décider de la direction.
    """
...
def main() -> int:
    ...
    # Fix F1 : signaux primaires Donchian sur test, puis méta-filtrage
    ...
    print(f"\n{'='*60}")
    print(f"Phase B C5 Extra — GBPUSD D1 terminé.")
