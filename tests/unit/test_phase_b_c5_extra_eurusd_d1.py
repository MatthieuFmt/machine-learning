"""Tests unitaires pour Phase B C5 Extra — EURUSD D1 (sans méta-labeling)."""
...
from scripts.run_phase_b_c5_extra_eurusd_d1 import (
...
def sample_ohlc() -> pd.DataFrame:
    ...
    n = 500
    base = 1.0800
    trend = np.linspace(0, 0.0600, n) + np.random.randn(n).cumsum() * 0.0015
    close = base + trend
    high = close + np.abs(np.random.randn(n)) * 0.0040
    low = close - np.abs(np.random.randn(n)) * 0.0040
    open_ = close - np.random.randn(n) * 0.0010
...
def test_c5_features_are_defined():
    """Les 15 features C5 pour EURUSD D1 sont définies dans FEATURES_SELECTED."""
...
def test_c5_features_no_duplicates():
    """Pas de doublons dans les features C5 EURUSD D1."""
    features = FEATURES_SELECTED[COUPLE_KEY]
    assert len(features) == len(set(features)), f"Doublons détectés dans {features}"


def test_asset_config_exists():
    """EURUSD est dans ASSET_CONFIGS."""
    assert "EURUSD" in ASSET_CONFIGS, "EURUSD absent de ASSET_CONFIGS"
...
def test_donchian_signals_breakout_pattern():
    ...
    close[-10:] = base + np.linspace(0.0040, 0.0120, 10)
    high = close + 0.0001
    low = close - 0.0001
    open_ = close
