"""Tests unitaires pour Phase B C5 — B4 USDCHF H4 (sans méta-labeling)."""
...
from scripts.run_phase_b_c5_b4_usdchf_h4 import (
...
def sample_ohlc() -> pd.DataFrame:
    ...
    n = 500
    base = 0.9200
    # Tendance forte + bruit élevé pour garantir des breakouts Donchian
...
def test_c5_features_are_defined():
    """Les 15 features C5 pour USDCHF H4 sont définies dans FEATURES_SELECTED."""
...
def test_c5_features_no_duplicates():
    """Pas de doublons dans les features C5 USDCHF H4."""
    features = FEATURES_SELECTED[COUPLE_KEY]
...
def test_train_rf_produces_predictions():
    """Le modèle rf entraîné produit des prédictions binaires valides."""
    np.random.seed(42)
    n_samples = 100
    n_features = 15
    X = pd.DataFrame(
...
def test_train_rf_binary_target():
...
def test_donchian_signals_format(sample_ohlc: pd.DataFrame):
    ...
    # Les premières barres (warmup N=20) doivent être 0
    assert (signals.iloc[:20] == 0).all(), "Les 20 premières barres devraient être 0 (warmup)"


def test_donchian_signals_breakout_pattern():
    """Un breakout clair (Close dépasse le max High récent) génère un signal LONG."""
    np.random.seed(42)
    n = 50
    base = 0.9200
    # 40 barres stables, puis breakout haussier brutal
    close = np.full(n, base)
    close[-10:] = base + np.linspace(0.0050, 0.0150, 10)  # breakout progressif
    high = close + 0.0001
    low = close - 0.0001
    open_ = close
