"""Tests unitaires pour Phase B C5 — B2 ETHUSD H1 (sans méta-labeling)."""
...
from scripts.run_phase_b_c5_b2_ethusd_h1 import (
...
def sample_ohlc() -> pd.DataFrame:
    """Génère 500 barres H1 synthétiques avec tendance haussière légère."""
    np.random.seed(42)
    n = 500
    base = 1800.0
...
def sample_trades() -> list[dict]:
    ...
    return [
        {"entry_time": "2022-01-03 10:00:00+00:00", "exit_time": "2022-01-03 14:00:00+00:00",
         "signal": 1, "entry_price": 1800.0, "exit_price": 1900.0,
         "pips_net": 100.0, "result": "win"},
        {"entry_time": "2022-01-04 11:00:00+00:00", "exit_time": "2022-01-04 15:00:00+00:00",
         "signal": -1, "entry_price": 1900.0, "exit_price": 1950.0,
         "pips_net": -50.0, "result": "loss_sl"},
        {"entry_time": "2022-01-05 09:00:00+00:00", "exit_time": "2022-01-05 13:00:00+00:00",
         "signal": 1, "entry_price": 1850.0, "exit_price": 1950.0,
         "pips_net": 100.0, "result": "win"},
        {"entry_time": "2022-01-06 12:00:00+00:00", "exit_time": "2022-01-06 16:00:00+00:00",
         "signal": -1, "entry_price": 1950.0, "exit_price": 1900.0,
         "pips_net": 50.0, "result": "win"},
        {"entry_time": "2022-01-07 08:00:00+00:00", "exit_time": "2022-01-07 12:00:00+00:00",
         "signal": 1, "entry_price": 1920.0, "exit_price": 1890.0,
         "pips_net": -30.0, "result": "loss_sl"},
    ]
...
def test_c5_features_are_defined():
    """Les 15 features C5 pour ETHUSD H1 sont définies dans FEATURES_SELECTED."""
...
def test_c5_features_no_duplicates():
    """Pas de doublons dans les features C5 ETHUSD H1."""
    features = FEATURES_SELECTED[COUPLE_KEY]
    assert len(features) == len(set(features)), f"Doublons détectés dans {features}"


def test_asset_config_exists():
    """ETHUSD est dans ASSET_CONFIGS."""
    assert "ETHUSD" in ASSET_CONFIGS, "ETHUSD absent de ASSET_CONFIGS"


# ── Test 2 : Modèle produit des prédictions ───────────────────────────────────

def test_train_hgbm_produces_predictions():
    """Le modèle hgbm entraîné produit des prédictions binaires valides."""
    np.random.seed(42)
    n_samples = 100
    n_features = 15
    X = pd.DataFrame(
...
def test_train_hgbm_binary_target():
    ...
    model = _train_hgbm_model(X, y)
    preds = model.predict(X.values)
    # Avec class_weight='balanced', au moins une prédiction de chaque classe
    unique = set(preds)
    assert len(unique) >= 1  # au moins une classe prédite
...
def test_donchian_signals_not_all_zero(sample_ohlc: pd.DataFrame):
    """Avec 500 barres de données variées, on doit avoir des signaux non nuls."""
    signals = _generate_donchian_signals(sample_ohlc)
    n_signals = int((signals != 0).sum())
    assert n_signals > 0, f"Aucun signal Donchian généré sur {len(sample_ohlc)} barres"
