"""Tests unitaires pour Phase B C5 Extra — ETHUSD H4 (sans méta-labeling)."""
...
from scripts.run_phase_b_c5_extra_ethusd_h4 import (
...
def sample_ohlc() -> pd.DataFrame:
    """Génère 500 barres H4 synthétiques avec tendance haussière volatile."""
    np.random.seed(42)
    n = 500
    base = 1800.0
    trend = np.linspace(0, 300, n) + np.random.randn(n).cumsum() * 15
    close = base + trend
...
def sample_trades() -> list[dict]:
    ...
    return [
        {"entry_time": "2022-01-03 08:00:00+00:00", "exit_time": "2022-01-03 16:00:00+00:00",
         "signal": 1, "entry_price": 1800.0, "exit_price": 1900.0,
         "pips_net": 100.0, "result": "win"},
        {"entry_time": "2022-01-04 08:00:00+00:00", "exit_time": "2022-01-04 16:00:00+00:00",
         "signal": -1, "entry_price": 1900.0, "exit_price": 1950.0,
         "pips_net": -50.0, "result": "loss_sl"},
        {"entry_time": "2022-01-05 08:00:00+00:00", "exit_time": "2022-01-05 16:00:00+00:00",
         "signal": 1, "entry_price": 1850.0, "exit_price": 1950.0,
         "pips_net": 100.0, "result": "win"},
        {"entry_time": "2022-01-06 08:00:00+00:00", "exit_time": "2022-01-06 16:00:00+00:00",
         "signal": -1, "entry_price": 1950.0, "exit_price": 1900.0,
         "pips_net": 50.0, "result": "win"},
        {"entry_time": "2022-01-07 08:00:00+00:00", "exit_time": "2022-01-07 16:00:00+00:00",
...
def test_c5_features_are_defined():
    """Les 15 features C5 pour ETHUSD H4 sont définies dans FEATURES_SELECTED."""
...
def test_c5_features_no_duplicates():
    """Pas de doublons dans les features C5 ETHUSD H4."""
    features = FEATURES_SELECTED[COUPLE_KEY]
...
def test_donchian_signals_breakout_pattern():
    ...
    close[-10:] = base + np.linspace(50, 150, 10)
    high = close + 1.0
    low = close - 1.0
    open_ = close

    idx = pd.date_range("2000-01-01", periods=n, freq="4h", tz="UTC")
