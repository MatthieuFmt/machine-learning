"""Tests unitaires pour Phase B C5 — B3 ETHUSD D1 (sans méta-labeling)."""
...
from scripts.run_phase_b_c5_b3_ethusd_d1 import (
...
def sample_ohlc() -> pd.DataFrame:
    ...
    trend = np.linspace(0, 500, n) + np.random.randn(n).cumsum() * 20
    close = base + trend
...
def test_c5_features_are_defined():
    """Les 15 features C5 pour ETHUSD D1 sont définies dans FEATURES_SELECTED."""
...
def test_c5_features_no_duplicates():
    """Pas de doublons dans les features C5 ETHUSD D1."""
    features = FEATURES_SELECTED[COUPLE_KEY]
...
def test_donchian_signals_breakout_pattern():
    ...
    close[-10:] = base + np.linspace(50, 150, 10)  # breakout progressif
    high = close + 1.0
    low = close - 1.0
    open_ = close
