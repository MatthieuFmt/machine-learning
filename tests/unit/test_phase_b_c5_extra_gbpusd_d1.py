"""Tests unitaires pour Phase B C5 Extra — GBPUSD D1 (sans méta-labeling)."""
...
from scripts.run_phase_b_c5_extra_gbpusd_d1 import (
...
def sample_ohlc() -> pd.DataFrame:
    ...
    n = 500
    base = 1.2500
    trend = np.linspace(0, 0.0800, n) + np.random.randn(n).cumsum() * 0.0020
    close = base + trend
...
def test_c5_features_are_defined():
    """Les 15 features C5 pour GBPUSD D1 sont définies dans FEATURES_SELECTED."""
...
def test_c5_features_no_duplicates():
    """Pas de doublons dans les features C5 GBPUSD D1."""
    features = FEATURES_SELECTED[COUPLE_KEY]
    assert len(features) == len(set(features)), f"Doublons détectés dans {features}"


def test_asset_config_exists():
    """GBPUSD est dans ASSET_CONFIGS."""
    assert "GBPUSD" in ASSET_CONFIGS, "GBPUSD absent de ASSET_CONFIGS"
