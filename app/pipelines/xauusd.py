"""Pipeline XAUUSD H4 — Trend-Following mono-TF, cible binaire symétrique.

Hypothèse v2-02 :
- Cible : directional_clean (binaire symétrique, horizon 96h = 24 barres H4, bruit 0.5×ATR)
- Features : 5 mono-H4, pas de D1, pas de macro, pas de calendrier
- Modèle : RF 300 arbres, depth=5, leaf=50, class_weight=balanced_subsample
- Split : train ≤ 2022-12-31, val 2023, test 2024-2025
- Backtest : TP=300, SL=150, commission=25, slippage=10, threshold=0.45, vol_filter only
"""
...
from app.config.instruments import XauUsdConfig
from app.core.logging import get_logger
from app.targets.labels import compute_directional_clean
...
class XauUsdPipeline:
    """Pipeline XAUUSD H4 — v2 Hypothesis 02.

    Mono-TF H4 strict, 5 features exactement, RF fortement régularisé,
    protocole anti-snooping strict (split figé, un seul regard).
    D1 chargé mais pas utilisé dans build_features.
    """

    def __init__(self) -> None:
        self.instrument = XauUsdConfig()
        self.backtest_cfg = BacktestConfig(
            tp_pips=300.0,
            sl_pips=150.0,
            window_hours=96,
            commission_pips=25.0,
            slippage_pips=10.0,
    ...
    def load_data(self) -> dict[str, Any]:
        """Charge XAUUSD H4 et D1 depuis cleaned-data/.

        D1 est chargé mais pas utilisé dans build_features (gardé pour v2-02b).
        """
        ...
        for tf in ("H4", "D1"):
            path = cleaned_dir / f"XAUUSD_{tf}_cleaned.csv"
            if not path.exists():
                raise FileNotFoundError(
                    f"{path} introuvable. Exécuter d'abord : "
                    f"python scripts/inspect_xauusd_csv.py"
                )
            df = pd.read_csv(path, index_col=0, parse_dates=True)
            logger.info(
                "Chargé %s : %d barres, %s → %s",
                path.name,
                len(df),
                df.index.min().strftime("%Y-%m-%d"),
                df.index.max().strftime("%Y-%m-%d"),
            )
            data[f"xauusd_{tf.lower()}"] = df
    ...
    def build_features(
        ...
        """Construit 5 features mono-H4 + Target directional_clean.

        Features (H4 uniquement) :
        - RSI_14, ADX_14, Dist_SMA50, ATR_Norm, Log_Return_24
        """
        h4 = data["xauusd_h4"].sort_index()
        close = h4["Close"].values.astype(np.float64)
        high = h4["High"].values.astype(np.float64)
        low = h4["Low"].values.astype(np.float64)
        n = len(close)

        ml = pd.DataFrame(index=h4.index)
        ...
        # ── Target : directional_clean, horizon 96h = 4 jours ──
        ml["Target"] = compute_directional_clean(
            h4, horizon_hours=96, noise_atr=0.5, atr_period=14,
        )
        ...
        # ── Feature 5 : Log_Return_24 (momentum 4 jours = 24 barres H4) ──
        log_ret = np.full(n, np.nan)
        log_ret[24:] = np.log(close[24:] / close[:-24])
        ml["Log_Return_24"] = log_ret
    ...
    def train_model(
        ...
        """Entraîne RF sur train ≤ 2022-12-31."""
